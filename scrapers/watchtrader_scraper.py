#!/usr/bin/env python3
"""
Watch Trader UK Scraper
Scrapes watch listings from watchtrader.co.uk
"""
...
class WatchTraderScraper(BaseScraper):
    def __init__(self):
        super().__init__()
        self.site_name = "watchtrader"
        self.base_url = "https://www.watchtrader.co.uk"
        self.start_urls = [
            "https://www.watchtrader.co.uk/shop/",
            "https://www.watchtrader.co.uk/shop/page/2/",
            "https://www.watchtrader.co.uk/shop/page/3/",
            "https://www.watchtrader.co.uk/shop/page/4/",
            "https://www.watchtrader.co.uk/shop/page/5/"
        ]
    ...
    def extract_watch_info(self, soup, url):
        ...
        try:
            # Extract title
            title_elem = soup.find('h1', class_='product_title') or soup.find('h1', class_='entry-title')
            if title_elem:
                watch_data['title'] = title_elem.get_text(strip=True)

            # Extract price
            price_elem = soup.find('span', class_='price') or soup.find('span', class_='amount')
            if price_elem:
            ...
            brands = ['ROLEX', 'OMEGA', 'CARTIER', 'PATEK PHILIPPE', 'AUDEMARS PIGUET', 'BREITLING', 'TAG HEUER', 'TUDOR', 'SEIKO', 'TISSOT']
            for brand in brands:
                if brand in title:
            ...
            for img in img_elements:
                src = img.get('src')
                if src and ('product' in src.lower() or 'watch' in src.lower()):
                    if src.startswith('//'):
                        src = 'https:' + src
                    elif src.startswith('/'):
    ...
    def scrape_product_urls(self, category_url, max_products=50):
    ...
    def scrape_site(self, max_products_per_page=20):
        ...
        watches = []
        for i, url in enumerate(all_urls[:max_products_per_page * len(self.start_urls)], 1):
            try:
...
def main():
    scraper = WatchTraderScraper()
    watches = scraper.scrape_site(max_products_per_page=15)
